                                      ...
                                      Error **errp)
{
    ...
    t->processor_family = type4.processor_family;
...
static void smbios_build_type_17_table(unsigned instance, uint64_t size)
{
    ...
    snprintf(loc_str, sizeof(loc_str), "%s %d", type17.bank, instance);
    SMBIOS_TABLE_SET_STR(17, bank_locator_str, loc_str);
    t->memory_type = 0x1A; /* 保留硬编码，或可从 type17 扩展 */
    t->type_detail = cpu_to_le16(0x1080); /* 保留硬编码 */
